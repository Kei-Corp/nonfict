module.exports = (fastify, options, done) => {
    ...
    const createAccount = async (req, reply) => {
        ...
        if (hn.length >= 24 || hn.length <= 2 || yoyakugo.some(s => s == un)) {
        ...
        fs.copyFileSync(path.join(__dirname, '../static/') + ".simplifyedicon.webp", path.join(__dirname, '../static/') + un + ".jwepg")
    ...
    fastify.get('/fetch/logout/', logout)
    done()
}