...
module.exports = (fastify, options, done) => {
    ...
    const formatTime2 = t => `${t.getFullYear()}年${t.getMonth()+1}月${t.getDate()}日 ${t.getHours()}:${t.getMinutes()}`
    ...
    const user = async (req, reply) => {
        if("secret_key" in req.cookies && await redis.exists(req.cookies.secret_key) && await redis.get(req.cookies.secret_key) == req.cookies.username){
            ...
            let html = u
                        ...
                        .replaceAll("[[bio]]", (await redis.get(username + ":bio")).replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("&lt;br&gt;", "<br>").replaceAll(/((?<!href="|href='|src="|src=')(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig, "<a href='$1' class='text-emerald-500'>$1</a>"))
    ...
    const edit = async (req, reply) => {
        if("secret_key" in req.cookies && await redis.exists(req.cookies.secret_key) && await redis.get(req.cookies.secret_key) == req.cookies.username){
            await redis.set(req.cookies.username + ":handlename", JSON.parse(req.body)["handlename"].slice(0, 30).replaceAll("<", "&lt;").replaceAll(">", "&gt;"))
            await redis.set(req.cookies.username + ":bio", JSON.parse(req.body)["bio"].slice(0, 500).replace(/\n{3,}/g, "\n\n").replace(/([ 　])[ 　]+/g, "$1").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br>"))
    ...
    const iwasreply = async (req, reply) => {
        ...
        if(await redis.exists(req.cookies.secret_key) && await redis.get(req.cookies.secret_key) == req.cookies.username && req.body.trim() && typingarray[id].hasOwnProperty(req.cookies.username)){
            ...
            let uhjk = req.cookies.username + await redis.get(req.cookies.username + ":handlename") + dats.c.slice(0, 1024)
            if(uhjk.length >= 2){
    ...
    const followers_list_html = async (req, reply) => {
        if("secret_key" in req.cookies && await redis.exists(req.cookies.secret_key) && await redis.get(req.cookies.secret_key) == req.cookies.username){
            ...
            const L = (await redis.smembers(target + ":followers")).slice().reverse()
            let innerhtmlt = ""
            if(L.length != 0){
                const L2 = await redis.mget(L.map(_ => _ + ":handlename"))
                const L3 = await redis.mget(L.map(_ => _ + ":bio"))
                for(i in L){
                    innerhtmlt += kkk.replaceAll("[[username]]", L[i]).replaceAll("[[handlename]]", L2[i]).replaceAll("[[bio]]", L3[i].replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("&lt;br&gt;", "<br>"))
                    .replaceAll("[[button]]", (L[i] == un) ? "" : (!await redis.sismember(L[i] + ":followers", un) ? `<button name="${L[i]}" class="follow-up-button absolute right-6 top-2 hover:bg-emerald-600 box-border block text-lg py-0.5 px-4 font-semibold bg-emerald-500 text-white text-center">
                    フォロー
    ...
    const followees_list_html = async (req, reply) => {
        if("secret_key" in req.cookies && await redis.exists(req.cookies.secret_key) && await redis.get(req.cookies.secret_key) == req.cookies.username){
            ...
            let hn = await redis.get(un + ":handlename")
            let hnt = await redis.get(target + ":handlename")
            const L = (await redis.smembers(target + ":followees")).slice().reverse()
            let innerhtmlt = ""
            if(L.length != 0){
                const L2 = await redis.mget(L.map(_ => _ + ":handlename"))
                const L3 = await redis.mget(L.map(_ => _ + ":bio"))
                for(i in L){
                    innerhtmlt += kkk.replaceAll("[[username]]", L[i]).replaceAll("[[handlename]]", L2[i]).replaceAll("[[bio]]", L3[i].replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("&lt;br&gt;", "<br>"))
                    .replaceAll("[[button]]", (L[i] == un) ? "" : (!await redis.sismember(L[i] + ":followers", un) ? `<button name="${L[i]}" class="follow-up-button absolute right-6 top-2 hover:bg-emerald-600 box-border block text-lg py-1 px-4 font-semibold bg-emerald-500 text-white text-center">
                    フォロー
                </button>`: `<button name="${L[i]}" class="follow-up-button absolute right-6 top-2 hover:border-emerald-600 box-border block text-lg py-0.5 px-4 font-semibold border-emerald-500 border-2 text-emerald-500 text-center">
                    リムーブ
                </button>`))
