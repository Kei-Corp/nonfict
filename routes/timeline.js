...
module.exports = (fastify, options, done) => {
    ...
    const post = async (req, reply) => {
        if("secret_key" in req.cookies && await redis.exists(req.cookies.secret_key) && await redis.get(req.cookies.secret_key) == req.cookies.username && req.body.trim() && typingarray.hasOwnProperty(req.cookies.username) && typinghistory.hasOwnProperty(req.cookies.username) && typinghistory[req.cookies.username].length >= 3 && (typinghistory[req.cookies.username].slice(-1)[0][1] - typinghistory[req.cookies.username].slice(0)[0][1]) >= 500){
    ...
    const getTimeline = async (redis, heis = null, start = 0, end = 16) => {
