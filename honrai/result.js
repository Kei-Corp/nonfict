...
render(JSON.parse(megens))
if(megens == "{}"){
    document.getElementById("megens").innerHTML="<div class='text-sm mt-6 text-slate-500 text-center'>一件も投稿がありません……；；</div>"
}

ws2 = new ReconnectingWebSocket(`wss://${location.hostname}/ws/screenshare/${getCookie("username")}/${getCookie("secret_key")}/`)

let aer = 0
ws2.addEventListener("open", ()=>{ws2.send(location.pathname + location.search); ws2.send(JSON.stringify([window.innerWidth, window.innerHeight, scrollY]))})
setInterval(()=>{ws2.send(location.pathname + location.search);}, 5000)
window.addEventListener("focus", () => {ws2.send(location.pathname + location.search);})
